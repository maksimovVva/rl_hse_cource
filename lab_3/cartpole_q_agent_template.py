...
def build_state(observation):
    ...
    l = [discretize_value(feature, state_bins[i]) * ((max_bins + 1) ** i) for i, feature in enumerate(observation)]
    state = sum(l)
    return state
...
if __name__ == '__main__':
    ...
    agent = QLearningAgent(alpha=0.5, epsilon=0.1, discount=0.95, get_legal_actions=lambda s: range(n_actions))
